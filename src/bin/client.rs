...
fn main() {
    ...
    println!("\n=== NTLMPassport End-to-End Handshake ===");
    test_ntlm_passport();
}
...
fn test_ntlm() {
    ...
    assert!(reply1.starts_with(b"AUTH NTLM S :"));
    ...
    assert!(reply2.starts_with(b"AUTH NTLM * "));
    assert!(reply2.ends_with(b" 0"));
    println!("NTLM Handshake successful!");
}

fn test_ntlm_passport() {
    let mut stream = TcpStream::connect("127.0.0.1:6667").expect("Failed to connect to server");
    println!("Connected to server on port 6667.");

    let ntlm_gkp_provider = NtlmPassportSecurityProvider::new();
    let mut client_cred = CredHandle::default();
    ntlm_gkp_provider.acquire_credentials_handle(None, "NTLMPassport", 1, None, &mut client_cred).unwrap();
    ...
    let res_c1 = ntlm_gkp_provider.initialize_security_context(
        &client_cred,
        None,
        None,
        0,
    ...
    let mut msg1 = b"AUTH NTLMPassport I :".to_vec();
    msg1.extend_from_slice(&escaped_token_1);
    msg1.extend_from_slice(b"\r\n");
    println!("Client: {}", String::from_utf8_lossy(&msg1).trim());
    stream.write_all(&msg1).unwrap();
    ...
    assert!(reply1.starts_with(b"AUTH NTLMPassport S :"));
    let escaped_challenge = &reply1[b"AUTH NTLMPassport S :".len()..];
    let challenge_token = unescape(escaped_challenge);
    ...
    let res_c2 = ntlm_gkp_provider.initialize_security_context(
        &client_cred,
        Some(&client_ctx),
        None,
        0,
    ...
    let mut msg2 = b"AUTH NTLMPassport S :".to_vec();
    msg2.extend_from_slice(&escaped_token_2);
    msg2.extend_from_slice(b"\r\n");
    println!("Client: {}", String::from_utf8_lossy(&msg2).trim());
    stream.write_all(&msg2).unwrap();
    ...
    assert!(reply2.starts_with(b"AUTH NTLMPassport S :"));
    let escaped_ok = &reply2[b"AUTH NTLMPassport S :".len()..];
    let ok_bytes = unescape(escaped_ok);
    assert_eq!(ok_bytes, b"OK");

    // Initiate Passport Phase
    let passport_ticket = vec![0xCCu8; 200];
    let client_input_p = vec![
        SecBuffer { buffer_type: SecBufferType::Token, bytes: b"OK".to_vec() },
        SecBuffer { buffer_type: SecBufferType::Token, bytes: passport_ticket },
    ];
    ...
    let res_c3 = ntlm_gkp_provider.initialize_security_context(
        &client_cred,
        Some(&client_ctx_2),
        None,
        0,
    ...
    let mut msg3 = b"AUTH NTLMPassport S :".to_vec();
    msg3.extend_from_slice(&escaped_token_3);
    msg3.extend_from_slice(b"\r\n");
    println!("Client: {}", String::from_utf8_lossy(&msg3).trim());
    stream.write_all(&msg3).unwrap();
    ...
    assert!(reply3.starts_with(b"AUTH NTLMPassport * "));
    assert!(reply3.ends_with(b" 0"));
    println!("NTLMPassport Handshake successful!");
}
